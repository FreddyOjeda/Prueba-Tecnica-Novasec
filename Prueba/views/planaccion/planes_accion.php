...
<div class="container mt-4">
    <h2>Planes de Acción para el <?= $titulo ?> con ID: <?= $id ?></h2>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="index.php?entity=<?= $entity ?>&action=planes_accion&id=<?= $id ?>&delete_plan=<?= $plan['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Está seguro de eliminar este plan de acción?')">Eliminar</a>
                </td>
            </tr>
            ...
            <div class="modal fade" id="modalEditarPlan<?= $plan['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="modalEditarPlanLabel<?= $plan['id'] ?>" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action="index.php?entity=<?= $entity ?>&action=planes_accion&id=<?= $id ?>" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="id_usuario" required>
                                        <?php foreach ($usuarios as $usuario): ?>
                                            <option value="<?= $usuario['id'] ?>" <?= $usuario['id'] == $plan['id_usuario'] ? 'selected' : '' ?>>
                                                <?= $usuario['nombre'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="id_estado" required>
                                        <?php foreach ($estados as $estado): ?>
                                            <option value="<?= $estado['id'] ?>" <?= $estado['id'] == $plan['id_estado'] ? 'selected' : '' ?>>
                                                <?= $estado['nombre'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
    ...
    <a href="index.php?entity=<?= $entity ?>&action=index" class="btn btn-secondary">Volver</a>
</div>
